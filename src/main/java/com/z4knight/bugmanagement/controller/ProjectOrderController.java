...
public class ProjectOrderController {
    ...
    public Result selectByGroupId(@RequestParam(value = "orderId") String orderId) {
        try {
            // 请求成功，则按接口定义，返回成功信息以及数据
    ...
    public Result updateByGroupId(@RequestBody ProjectOrderForm projectOrderForm) {
        try {
            // 对输入数据进行校验
    ...
    public Result deleteByGroupId(@RequestParam(value = "orderId") String orderId) {
        try {
            // 请求成功，则按接口定义，返回成功信息以及数据
